...
function ListProductCategory() {
    ...
    const handleCheckedAll = () => {
        ...
        setIsCheck(() => productCategories.map((item) => item.id))
        if (isCheckAll) {
            setIsCheck([])
        }
    }
    ...
    useLayoutEffect(() => {
        if (!debouce.trim()) {
            setSearchResults(productCategories)
            return
        }
        const data = productCategories.filter(function (category) {
    ...
    return (
        <div className={cx('wrapper')}>
            <div className={cx('title')}>Danh sách danh mục sản phẩm</div>
            <Dialog
                open={openDialog}
                onClose={handleClose}
            ...
            <div className={cx('btn-group')}>
                <div className={cx('btn')}>
                    <Button primary to={admin.newProductCategory}>
            ...
            <div className={cx('content')}>
                ...
                <div className={cx('body', 'scrollbar-custom')}>
                    <div className={cx('list-categories')}>
                        {searchResults.map((productCategory, index) => (
                            <ProductCategoryItem
                                key={index}
                                productCategory={productCategory}
                                index={index}
                                isChecked={isCheck.includes(productCategory.id)}
                                Save={Save}
                                imageSrc={productCategory.thumbnail}
                                handleChecked={(e) => handleChecked(productCategory.id, e)}
                            />
                        ))}
                    </div>
                </div>
